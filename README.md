...
The alias therefore dangles in a fresh checkout (its target is a local, ignored install): prod skips
dangling relative in-repo symlinks when building a run view, and brain lint only WARNs. Ignored kit
paths do not follow a worktree — the installer lists them in the repo-root `.worktreeinclude`
allowlist; otherwise re-run `install.sh <worktree-dir>` inside the worktree.
...
| [`brain-source-intake`](skills/brain-source-intake/SKILL.md) | A codebase that became readable (source mirror or local clone) turned into brain knowledge: bounded orientation scan, benchmark of real customer questions against code and DB, self-contained questionnaire for the customer's developer (one copy-as-markdown button), answers written back as a `skills/codebase/` map. |
| [`brain-schema-intake`](skills/brain-schema-intake/SKILL.md) | A grounding database that became readable turned into brain knowledge: a bounded schema probe on the box, a benchmark of real customer questions into db / kb / both / human, a self-contained questionnaire for the customer's developer, answers written back as `skills/databases/<db>.md`. |
...
| `rc dev brain publish\|promote --channel <channel> --sha <sha>` | Moves a shared project brain channel for all its runs/tenants; project-maintainer only and audited. |
...
- `rc dev brain sync` refreshes the deployed brain cache from `origin/main` and invalidates warm console
  workspaces.
- `rc dev brain publish --channel stable|edge --sha <exact-full-40-character-sha>` (rc >= 1.16.5) is the
  preferred one-shot sync + promote + verify for a shared project channel; `rc dev brain preflight`
...
| Path | What |
|---|---|
| [docs/brain-model.md](docs/brain-model.md) | Brain boundary, `include_in`, `.replypenignore`, layout, mounts, and refs. |
...
together; see [RELEASING.md](RELEASING.md). Current line: **`v0.9.14`**.

- Runtime pin:
  `rootcause-runtime @ git+https://github.com/rootcause-org/rootcause-brain-skills@v0.9.14#subdirectory=runtime`
- Workspace image: `ghcr.io/rootcause-org/workspace:v0.9.14`
