...
1. **Publish this repo** at `v0.8.6` with `./refresh-brains.sh --release patch` (or an explicit
   ...
   - Prove the package resolves by tag (no `rootcause` source):
     ```bash
     uv run --no-project \
       --with "rootcause-runtime @ git+https://github.com/rootcause-org/rootcause-brain-skills@v0.8.6#subdirectory=runtime" \
       python -c "import lib.db; print('ok')"
     ```

2. **Publish the workspace image** to ghcr, pinned to the same tag:
   ```bash
   docker build -f docker/Dockerfile -t ghcr.io/rootcause-org/workspace:v0.8.6 .
   docker push ghcr.io/rootcause-org/workspace:v0.8.6
   ```
