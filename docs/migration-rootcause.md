...
1. **Tag this repo** `v0.1.82` and push.
   ...
   - Prove the package resolves by tag (no `rootcause` source):
     ```bash
     uv run --no-project \
       --with "rootcause-runtime @ git+https://github.com/rootcause-org/rootcause-brain-skills@v0.1.82#subdirectory=runtime" \
       python -c "import lib.db; print('ok')"
     ```

2. **Publish the workspace image** to ghcr, pinned to the same tag:
   ```bash
   docker build -f docker/Dockerfile -t ghcr.io/rootcause-org/workspace:v0.1.82 .
   docker push ghcr.io/rootcause-org/workspace:v0.1.82
   ```
...
3. **Repoint prod** — `rootcause/runtime/Dockerfile`. Replace the inline client-dep install +
   ...
   +RUN uv pip install --system --no-cache \
   +        "rootcause-runtime @ git+https://github.com/rootcause-org/rootcause-brain-skills@v0.1.82#subdirectory=runtime"
   ```
