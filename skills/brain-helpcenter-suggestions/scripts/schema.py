...
Verdict = Literal["answered", "partial", "missing", "wrong_title", "not_kb", "uncertain"]
Kind = Literal["new", "rewrite", "retitle", "merge", "delete", "add_alias"]
GAP_VERDICTS = ("partial", "missing", "wrong_title", "uncertain")
WEIGHT = {"missing": 3, "partial": 2, "wrong_title": 1, "uncertain": 1}
...
def _cross(sug: Suggestions, ev: Evidence, evidence_path: Path) -> list[str]:
    out: list[str] = []
...
def _check_suggestion(p, s, arts, convs, seen, topics) -> list[str]:
    out: list[str] = []
    ...
    if s.kind == "new":
        ...
        if not s.section:
            out.append(f"{p}.section: kind rewrite requires the section to replace")
        if not s.text:
            out.append(f"{p}.text: kind rewrite requires the replacement text")
    elif s.kind == "retitle":
        if n != 1:
...
def soft_warnings(sug: Suggestions, ev: Evidence) -> list[str]:
    out: list[str] = []
...
def summary(ev: Evidence, sug: Suggestions) -> dict[str, Any]:
    counts = {v: 0 for v in ("answered", "partial", "missing", "wrong_title", "not_kb", "uncertain")}
...
def validation_errors(suggestions_path: str | Path, evidence_path: str | Path) -> list[str]:
    """Actionable `json.path: message (hint)` lines; empty means valid."""
    ev_path = Path(evidence_path)
    errors, ev = _load(ev_path, Evidence, "evidence.json")
    if errors:
        return [f"evidence.{e}" if not e.startswith("<") else f"evidence.json {e}" for e in errors]
    errors, sug = _load(Path(suggestions_path), Suggestions, "suggestions.json")
    if errors:
        return errors
    return _cross(sug, ev, ev_path)


def load(suggestions_path: str | Path, evidence_path: str | Path) -> tuple[Suggestions, Evidence]:
    errors = validation_errors(suggestions_path, evidence_path)
    if errors:
        raise ValueError("\n".join(errors))
