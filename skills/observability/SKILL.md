...
only outside a brain checkout or to override it. On tenant-enabled projects, omit `--tenant` by default;
the active `rc login` supplies the tenant. If a RootCause MCP is installed, ignore it unless the
