...
"""One window of real customer questions, reduced to `questions.tsv` in the intake dir.
    ...
    uv run skills/brain-source-intake/scripts/questions.py [--days 60] [--out DIR]
    uv run skills/brain-source-intake/scripts/questions.py --from FILE [--out DIR]

`--days` runs the sibling `brain-helpcenter-suggestions` collector (read-only `rc`) into
...
def run_collector(brain_root: Path, days: int, raw: Path) -> Path:
    if not COLLECTOR.is_file():
        print(f"questions: the sibling collector is missing ({COLLECTOR}). "
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = Path(args.out or brain_root / ".rootcause" / "source-intake" / date.today().isoformat())
