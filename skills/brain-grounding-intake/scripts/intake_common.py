...
# dependencies = []
# ///
"""Pieces the intake scripts share: TSV reading, frontmatter, markdown hygiene.

`validate.py` imports this directly (same directory). Keep it free of anything specific to
the code half or the data half, so both are held to the same rules.
"""
...
def check_markdown_hygiene(label: str, text: str, errors: list[str],
                           max_lines: int, max_line_chars: int,
    ...
    An over-long line is a warning when `warnings` is given (a routing table row can be long),
    an error otherwise."""
    lines = text.split("\n")
    ...
    for number, line in enumerate(lines, start=1):
        ...
        if len(line) > max_line_chars:
            (warnings if warnings is not None else errors).append(
                f"{label}: line {number}: {len(line)} characters, over {max_line_chars}; "
                "split it or list as bullets")
        if "\u2014" in line:
