...
"""Ship `probe.py` to the box, fetch its JSON, reduce it to the read-whole `schema.md`.
    ...
    uv run skills/brain-schema-intake/scripts/collect.py --db staging [--out DIR]
    uv run skills/brain-schema-intake/scripts/collect.py --from schema.json --out DIR
...
if not (SIBLING / "scan.py").is_file():
    print(f"collect: the sibling brain-source-intake scripts are missing ({SIBLING}). "
...
def console_bash(brain_root: Path, command: str, timeout: int) -> None:
    proc = subprocess.run(
    ...
    if proc.returncode != 0 and not result:
        ...
        raise SystemExit(f"collect: `rc dev console bash run` failed (exit {proc.returncode})")
    if result.get("timed_out"):
        raise SystemExit(f"collect: the probe hit the {timeout}s console timeout. Rerun with a "
                         "larger --timeout, or a smaller --profile-max-rows.")
    if result.get("exit_code"):
        sys.stderr.write(str(result.get("stderr") or ""))
        raise SystemExit(f"collect: the probe exited {result['exit_code']} on the box")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--out", type=Path, help="output directory, default .rootcause/schema-intake/<date>")
    ...
    out_dir = Path(args.out or brain_root / ".rootcause" / "schema-intake"
    ...
    if args.source:
        ...
        console_bash(brain_root, remote_command(args.db, remote,
                                                {"profile_max_rows": args.profile_max_rows}),
                     args.timeout)
