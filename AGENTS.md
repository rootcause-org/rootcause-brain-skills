...
- **Local brain development:** a developer or coding agent runs outside production from a brain
  ...
  locally into the ignored Codex-native `.agents/skills/` tree; `.claude/skills` is a relative symlink
  to that tree (Claude Code discovers only `.claude/skills`). Alias and skills are both LOCAL and
  IGNORED — committing the alias ships a symlink whose target is untracked, which prod's run view
  materializes broken and brain lint's `symlink-broken` rule rejects. Git worktrees carry tracked files
  only, so `.worktreeinclude` carries these paths instead. They never mount into `/brain`.
